...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    build: "Express with TypeScript, backed by PostgreSQL using raw SQL queries. The architecture centres on three patterns: stateless JWT authentication with refresh token rotation, a role-based middleware layer that enforces permissions declaratively at the route level, and structured error handling with consistent response shapes regardless of failure type. The schema uses normalisation with foreign key constraints and indexes on frequently queried columns.",
    result: "A production-ready REST API with complete authentication, granular role-based authorisation, and clean endpoint structure. The codebase demonstrates backend engineering depth — thoughtful data modelling, security architecture, and API design without relying on framework abstractions.",
    stack: ["Express", "TypeScript", "PostgreSQL", "JWT", "REST API"],
    links: [{ label: "View Source", href: "https://github.com/rayhanhabib07/DevPulse" }],
  },
  {
    number: "03",
    name: "Agency Starter",
    ...
    approach: "Build an opinionated starter that works end-to-end out of the box: landing page, contact form with server-side validation, database storage, and email notifications. Every integration degrades gracefully — if a credential isn’t configured yet, that step is skipped instead of crashing. This lets developers ship immediately and wire up services incrementally.",
    build: "Next.js App Router with TypeScript and Tailwind CSS for the frontend. The contact form submits to a serverless API route that validates input with Zod, stores submissions in a Supabase Postgres table (with Row Level Security, no public access), and sends email notifications via Resend. Rate limiting (5 per IP per 10 minutes) is built in. The database migration lives in the repo and the schema enforces server-only access through RLS with zero policies.",
    result: "A deployable, production-grade agency starter used as the foundation for client projects. Live on Vercel with automatic CI/CD from GitHub. The contact pipeline handles validation, storage, and notifications — a complete backend in a single API route that just works.",
  ...
  return (
    <section id="work" className="py-24 px-6" aria-labelledby="work-heading">
      <div className="max-w-5xl mx-auto">
        <p className="text-xs uppercase tracking-[0.2em] text-[#8a8680] mb-3">Case Studies</p>
        <h2
          id="work-heading"
        ...
        <p className="text-[#8a8680] text-base mb-16 max-w-2xl">
        ...
        <div className="space-y-16">
          {caseStudies.map((study) => (
            <article key={study.name} className="rounded-xl border border-[#333330] bg-[#222220] p-6 sm:p-10">
              <div className="flex items-start gap-4 mb-8">
                <span className="text-xs font-mono text-[#c8b89a] mt-1.5">{study.number}</span>
                <div>
                  <h3
                    className="text-2xl sm:text-3xl font-normal text-[#e0ddd5]"
                    style={{ fontFamily: "var(--font-playfair), Georgia, serif" }}
                  >
                    {study.name}
                  </h3>
                  <p className="text-[#8a8680] text-sm mt-1">{study.tagline}</p>
                </div>
              </div>
              ...
              <div className="grid gap-8 sm:gap-10">
                {[
                  ...
                  <div key={s.label}>
                    <p className="text-xs uppercase tracking-[0.2em] text-[#c8b89a] mb-2">{s.label}</p>
                    <p className="text-[#8a8680] text-sm sm:text-base leading-relaxed">{s.text}</p>
                  </div>
                ))}

                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[#c8b89a] mb-3">The Build</p>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {study.stack.map((tech) => (
                      <span key={tech} className="px-2.5 py-1 rounded text-xs text-[#8a8680] border border-[#333330]">
                        {tech}
                      </span>
                    ))}
                  </div>
                  <p className="text-[#8a8680] text-sm sm:text-base leading-relaxed">{study.build}</p>
                </div>

                <div>
                  <p className="text-xs uppercase tracking-[0.2em] text-[#c8b89a] mb-2">The Result</p>
                  <p className="text-[#8a8680] text-sm sm:text-base leading-relaxed">{study.result}</p>
                </div>
              </div>

              <div className="flex gap-4 mt-10 pt-6 border-t border-[#333330]">
                {study.links.map((link) => (
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-sm text-[#c8b89a] hover:text-[#e0ddd5] transition-colors duration-200"
                    aria-label={`${link.label} for ${study.name} (opens in new tab)`}
                  >
                    {link.label === "View Source" ? <GitHubIcon /> : <ExternalLinkIcon />}
                    {link.label}
                  </a>
                ))}
              </div>
