  ...
  return (
    <section id="services" className="py-24 px-6" aria-labelledby="services-heading">
      <div className="max-w-5xl mx-auto">
        <p className="text-sm font-medium text-[#4A90E2] mb-3 tracking-wide">Services</p>
        <h2 id="services-heading" className="text-3xl sm:text-4xl font-bold text-[#1B2631] mb-4">
          What I Build
        </h2>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {services.map((service) => (
            <div
              key={service.name}
              className="flex flex-col p-6 sm:p-8 rounded-2xl border border-[#E5E7EB] bg-[#F9FAFB] hover:border-[#4A90E2]/40 transition-colors duration-300"
            >
              <div className="w-11 h-11 rounded-lg flex items-center justify-center text-[#4A90E2] bg-[#4A90E2]/10 mb-5">
                {service.icon}
              </div>
              <h3 className="text-lg font-semibold text-[#1B2631] mb-3">{service.name}</h3>
              <p className="text-sm text-[#6B7280] leading-relaxed mb-6 flex-1">{service.description}</p>
              <div>
                <p className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wider mb-2">Use cases</p>
                <ul className="space-y-1">
                  {service.useCases.map((uc) => (
                    <li key={uc} className="text-sm text-[#6B7280] flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-[#4A90E2] shrink-0" aria-hidden="true" />
                      {uc}
                    </li>
                  ))}
                </ul>
              </div>
              <a href="#contact" className="inline-flex items-center gap-1.5 text-sm font-medium text-[#4A90E2] hover:text-[#357ABD] transition-colors duration-200 mt-6">
                Get a quote
